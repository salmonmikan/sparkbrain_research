...
- MAIN latest commit consumed: `904e9e041a26d102d87f0a6ac088385619fad363`.
- SUB latest commit consumed during the run: `e8ab335ab67a81b9ab2fc299a0266b7b32fd5d6c`.

Current programme status remains `NO_HIGH_VALUE_OBJECT / experimental cognitive architecture testbed`. LP01 remains closed pre-formal with no formal identity. Its current reference code is explicit parent/ancestor bookkeeping, not a demonstrated native online causal-credit mechanism. No new formal scientific evidence appeared.
...
This raises the bar beyond provenance alone. A future SparkBrain object that retains opaque token identities, ancestry, concurrent alternatives, or winner/loser history is not yet outside standard formal machinery. `Historical lineage + competition/conflict` can be represented by occurrence/causal nets and event structures without a novel cognitive credit principle.

Prospective implication: if a native lineage mechanism independently appears, include an individual-token causal-net/event-structure baseline receiving the same event envelope and test whether the claimed history/competition effect is reconstructible as ordinary partial-order/conflict state.
...
This fills a reduction-ladder gap. Provenance/ancestry records participation; structural-model actual causality asks counterfactual responsibility. Causally consistent dynamic slicing supplies an intermediate ordinary baseline that follows actual execution dependencies while respecting concurrency and causal equivalence.

Prospective implication: a future lineage-credit claim should beat provenance/ancestry, actual-run dynamic dependence/slicing, and counterfactual actual-cause/responsibility. Exceeding ancestor lookup but being exactly recovered by a dynamic slice remains ordinary trace dependence.

Source:
- Perera, Garg & Cheney, *Causally Consistent Dynamic Slicing*, CONCUR 2016, DOI 10.4230/LIPIcs.CONCUR.2016.18.

### 3. Local/online/spiking credit assignment is too well populated to carry novelty by itself

Generalized Latent Equilibrium (GLE) derives fully local spatio-temporal credit assignment for physical dynamical neuronal networks from neuron-local mismatch signals, with phase-free continuous-time local plasticity. Spike-based Alignment Learning (SAL), published 2026-07-07, gives a synapse-local STDP-compatible mechanism for maintaining effective weight alignment in noisy spiking systems and explicitly addresses locality constraints in physical credit assignment.

Neither is claimed to be an architectural equivalent of SparkBrain's remaining lineage-specific hypothesis, and neither by itself solves preemption/actual-responsibility attribution. But `local`, `online`, `physical`, `spiking`, or `history-sensitive learning` cannot stand alone as novelty axes.

Prospective implication: any future Spark-specific causal-credit object must demonstrate lineage-selective, intervention-validated responsibility under matched local information beyond ordinary local physical credit rules.
