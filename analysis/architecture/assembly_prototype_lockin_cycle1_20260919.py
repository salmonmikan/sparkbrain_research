...
def verify_contract(contract: dict[str, Any]) -> None:
    ...
    if float(contract["native_path"]["similarity_threshold"]) != 0.66:
...
def episode_row(episode: Any, patterns: tuple[Any, ...]) -> dict[str, Any]:
    return {
...
def generate_corpus(seed: int, contract: dict[str, Any]) -> dict[str, Any]:
    ...
    for episode in training_episodes(seed=seed, count=adaptation_count):
        ...
        adaptation_rows.append(episode_row(episode, patterns))
    ...
    for episode in held_out_episodes(
        ...
        patterns = tuple(result.patterns)
        probes_rows.append(episode_row(episode, patterns))
    ...
    manifest = {
        ...
        "probe_count": len(probes_rows),
        "adaptation_pattern_count": sum(
            len(row["patterns"]) for row in adaptation_rows
        ),
        "probe_pattern_count": sum(len(row["patterns"]) for row in probes_rows),
    }
...
def reward(action: str | None, expected: str | None) -> float:
    if expected is None:
        return 0.0
...
def run_arm(
    ...
    for episode_index in order:
        ...
        active = strongest(activations)
        predictor.predict(active)
        action = action_policy.choose(active, explore=True)
        predictor.observe(active, episode.future_event)
        action_policy.reward(reward(action.action, episode.rewarded_action))

    assignments = final_assignments(memory, adaptation, threshold)
    mature_count = sum(
    ...
    for probe_index, (episode, patterns) in enumerate(probes):
        ...
        for pattern_index, pattern in enumerate(patterns):
            ...
            if activation is not None:
                activations.append(activation)
        active = strongest(activations)
        prediction = predictor.predict(active)
        action = action_policy.choose(active, explore=False)
        ...
        probe_rows.append(
            {
                ...
                "assembly_id": active.assembly_id if active is not None else None,
            }
        )
    ...
    return {
        ...
        "prediction_coverage": covered / count if count else 0.0,
        "prediction_accuracy": correct / count if count else 0.0,
    }


def coclustering_disagreement(
...
def build_comparator(
    ...
    for members in groups.values():
        ...
        component_id = "cc-" + digest(sorted(member_keys))[:12]
...
def summarize(raw_rows: list[dict[str, Any]], seeds: list[int]) -> dict[str, Any]:
    by_seed: dict[int, dict[str, int]] = {}
    for seed in seeds:
        pair_rows = [
            row
            for row in raw_rows
            if row["row_type"] == "pair" and row["seed"] == seed
        ]
        by_seed[seed] = {
            ...
            "prediction_support_pairs": sum(
                int(row["prediction_disagreement_count"]) >= 2
                for row in pair_rows
            ),
    ...
    elif all(value >= 8 for value in rep_counts) and all(
        value >= 8 for value in pred_counts
    ):
...
def execute(output_dir: Path, contract: dict[str, Any]) -> None:
    ...
    corpora = {seed: generate_corpus(seed, contract) for seed in seeds}
    ...
    corpus_manifest = {
        ...
        "seeds": {
            str(seed): corpora[seed]["manifest"]
            for seed in seeds
        },
    }
    write_json_fsync(output_dir / "corpus_manifest.json", corpus_manifest)

    raw_rows: list[dict[str, Any]] = []
    comparators: dict[str, Any] = {}
    pairs_per_seed = int(contract["order_family"]["pairs_per_seed"])
    for seed in seeds:
        adaptation = corpora[seed]["adaptation_objects"]
        probes = corpora[seed]["probe_objects"]
        comparator = build_comparator(adaptation, probes, threshold)
        ...
        for pair_index in range(pairs_per_seed):
            order_a, order_seed = derive_order(
            ...
            arm_a = run_arm(adaptation, probes, order_a, threshold)
            arm_b = run_arm(adaptation, probes, order_b, threshold)
            disagreement = coclustering_disagreement(
                arm_a["assignments"],
                arm_b["assignments"],
            )
            rep_divergent = (
                arm_a["mature_assembly_count"]
                != arm_b["mature_assembly_count"]
                or disagreement > 0.0
            )
            ...
            raw_rows.append(
                {
                    ...
                    "prediction_coverage_delta": (
                        arm_a["prediction_coverage"]
                        - arm_b["prediction_coverage"]
                    ),
                    "prediction_accuracy_delta": (
                        arm_a["prediction_accuracy"]
                        - arm_b["prediction_accuracy"]
                    ),
    ...
    write_jsonl_fsync(output_dir / "raw.jsonl", raw_rows)

    reloaded_rows = [
        json.loads(line)
        for line in (output_dir / "raw.jsonl")
        .read_text(encoding="utf-8")
        .splitlines()
        if line.strip()
    ]
    summary = summarize(reloaded_rows, seeds)
    ...
    summary["exploration_cycle"] = {
        "discovery_completed": 1,
        "architecture_completed": 1,
    }
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "artifacts/architecture/assembly_prototype_lockin_cycle1"
        ),
    )
    ...
    if args.preflight_only:
        print("preflight-ok")
        return
    execute(args.output_dir, contract)
