...
def verify_binding(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    for path_text, expected in contract["source_binding"].items():
        ...
        if observed != expected:
            raise RuntimeError(f"source blob mismatch for {path_text}: {observed} != {expected}")
...
def topology_metadata(brain: IntegratedV05Brain) -> dict[str, Any]:
    ...
    reservoir_ids = tuple(sorted(unit_id for unit_id in field.units if unit_id not in receptor_set))

    def extents(unit_ids: tuple[int, ...]) -> dict[str, float | None]:
        if not unit_ids:
            return {"min_x": None, "max_x": None, "min_y": None, "max_y": None}
        xs = [float(field.units[unit_id].x) for unit_id in unit_ids]
        ys = [float(field.units[unit_id].y) for unit_id in unit_ids]
        return {
            "min_x": min(xs),
            "max_x": max(xs),
            "min_y": min(ys),
    ...
    return {
        ...
        "coordinate_extents_all": extents(all_ids),
        "coordinate_extents_reservoir": extents(reservoir_ids),
...
def field_state_metadata(field_state: dict[str, Any]) -> dict[str, Any]:
    ...
    def extents(rows: list[dict[str, Any]]) -> dict[str, float | None]:
        if not rows:
            return {"min_x": None, "max_x": None, "min_y": None, "max_y": None}
    ...
    return {
        ...
        "coordinate_extents_all": extents(units),
        "coordinate_extents_reservoir": extents(reservoir),
...
def inspect_config(row: dict[str, Any], temp_dir: Path) -> dict[str, Any]:
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        except Exception as exc:  # diagnostic must record deterministic rejection
            construction_error = f"{type(exc).__name__}: {exc}"
        construction_warnings = [f"{item.category.__name__}: {item.message}" for item in caught]
    if brain is None:
        return {
            "label": row["label"],
            "declared_v05_config_tuple": [config.width, config.height, config.receptor_rows],
    ...
    roundtrip_tuple = [restored_config.width, restored_config.height, restored_config.receptor_rows]
    return {
        "label": row["label"],
        ...
        "realized_topology_signature": topology_signature(pre_metadata),
        ...
        "roundtrip_realized_topology_signature": topology_signature(restored_metadata),
        "roundtrip_declared_config_preserved": roundtrip_tuple == declared_tuple,
        "roundtrip_realized_topology_preserved": topology_signature(restored_metadata)
        == topology_signature(pre_metadata),
    }
...
def map_outcome(rows: list[dict[str, Any]], contract_hits: list[dict[str, Any]]) -> str:
    ...
    nested_matches_declared = all(
        row["nested_v04_config_tuple"] == row["declared_v05_config_tuple"] for row in rows
    )
    no_nondefault_warning = all(not row["construction_warnings"] for row in rows[1:])

    if all_realized_fixed and declared_preserved and realized_preserved and nested_matches_declared:
        if contract_hits:
    ...
    if declared_preserved and realized_preserved:
...
def run(output_dir: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="sparkbrain-v05-config-contract-") as temp_name:
    ...
    raw_records = [
        {
            ...
            "git_head": os.environ.get("GITHUB_SHA", subprocess.check_output(["git", "rev-parse", "HEAD"], text=True).strip()),
    ...
    summary = {
        ...
        "construction_error_count": sum(bool(row.get("construction_error")) for row in rows),
        "nondefault_warning_count": sum(len(row.get("construction_warnings", [])) for row in rows[1:]),
        "unique_realized_topology_signatures": sorted(
            {row["realized_topology_signature"] for row in rows if "realized_topology_signature" in row}
        ),
