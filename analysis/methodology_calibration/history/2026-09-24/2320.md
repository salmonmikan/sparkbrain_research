...
Overall remains `SLIGHTLY_TOO_PERMISSIVE`: canonical/FORMA L/Revisit boundaries remain strong, but end-to-end development semantics are not fully consistent because a RESULT_EXPOSED zero-credit Forge object accepted a science-affecting same-object repair.

## Input generations / authoritative reconstruction
...
- Fast Forge TH-002 branch: initial probe `a9f81a2e4a53d70a57d2d6f80313ba024340414f`; first exposed dead-end report `cadf88a54c2715715a6f23e71183318e332ffb0d`; matched-access comparator change `98ea88e70f6f28f3463339689572270183fd90cd`; final report head `7db8abb08e7862e3bb98c985f2e7ec4d81cd9d16`.
...
Because the user's calibration taxonomy explicitly classifies comparator and resource/privilege contract changes as `SCIENCE_AFFECTING_CHANGE`, the correct post-exposure handling would have been one of:

1. preserve `TH002-FORGE-001` as the first exposed result and create an explicit versioned revision such as a revision identity for the matched-access correction; or
2. create a fresh successor probe with the corrected comparator contract and zero inherited credit.

The same-object history is preserved by Git commits, so no historical result was erased. But preservation alone is not equivalent to versioning the scientific object. The final R128 description calling the construction simply fixed overlooks this intermediate post-outcome contract change.

Gate action: `TIGHTEN` for RESULT_EXPOSED development repair classification and TH-002 Forge execution fidelity. No retroactive rerun is recommended.
