...
def reserved_field():
    return build_physical_field(
        unit_count=48,
        directed_edges=((0, 36), (36, 0), (36, 37), (0, 1)),
        threshold=0.5,
        initial_weight=0.05,
...
class BoundaryInterventionTests(unittest.TestCase):
    ...
    def test_observer_failure_cannot_be_misclassified_as_native_guard(self):
        with patch(
            "sparkbrain.research.rv02_boundary_recruitment.PartialObservedField.run_until",
...
class BoundaryScoringTests(unittest.TestCase):
    def score(self, units):
        return score_probe(
            [
                {"time_ms": 100.0 if i == 0 else 105.0 + i, "unit_id": unit}
