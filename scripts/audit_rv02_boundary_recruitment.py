...
def reconstruct_score(probe, route, unit_count):
    ...
    return {'generated_units': visible, 'ordered_retention': matched / len(expected),
...
def verify_probe(probe, score, route, unit_count):
    ...
    require(probe['actual_arrivals'] < 4096 and probe['actual_spikes'] < 512,
    ...
    require(probe['observer_equivalence'] is True and
            probe['observed_state_hash'] == probe['reference_state_hash'],
            'observer noninterference')
    require(probe['probe_connection_hash_before'] == probe['probe_connection_hash_after'],
            'probe learning')
    return metrics


def audit_bundle(output, source_root=ROOT):
    runner = load_script('rd002_retained_verifier', source_root / 'scripts/run_rv02_boundary_recruitment.py')
    verified = runner.verify_bundle(output, source_root=source_root)
    manifest = json.loads((output/'manifest.json').read_text())
    rows = [json.loads(line) for line in gzip.decompress((output/'raw_cells.jsonl.gz').read_bytes()).splitlines()]
    old_compressed = (source_root/'artifacts/research/rv02/rd001/raw_cells.jsonl.gz').read_bytes()
    require(hashlib.sha256(old_compressed).hexdigest() == 'd6507a988a3a2763742714993d2b420f735bf9fc76b45c308ce26b397d96e07e',
            'retained RD001 compressed identity')
    old_raw = gzip.decompress(old_compressed)
    require(hashlib.sha256(old_raw).hexdigest() == 'd786fd242c1c6621b39a6700096536a96109a67333222304412be90e5e510d9d',
    ...
    require(manifest['cell_timeout_seconds'] == 60 and manifest['total_timeout_seconds'] == 600
            and manifest['memory_limit_bytes'] == 1024**3, 'manifest resource contract')
    ...
    for row in rows:
        ...
        require(cell['changed_hidden_edges'] == [] and cell['hidden_external_trace_count'] == 0,
                'hidden learning boundary')
        count = cell['audit']['unit_count']
        require([u['unit_id'] for u in cell['trained_state']['units']] == list(range(count)), 'trained units')
        require(all(u['base_threshold'] == .5 and u['potential'] == 0 and u['adaptation'] == 0
                    and u['spike_count'] == 0 for u in cell['trained_state']['units']), 'initial unit dynamics')
        ...
        for pair in cell['route_probes']:
            ...
            for edge in gained:
                if edge['source_id'] < 36 <= edge['target_id']:
                    changes.append({'edge': [edge['source_id'], edge['target_id']],
                                    'old_weight': edge['weight'], 'new_weight': edge['weight']*gain})
                    edge['weight'] *= gain
            require(pair['gain_interventions'] == changes, 'gain isolation records')
            def connection_digest(edges):
                return hashlib.sha256(json.dumps(edges, sort_keys=True, separators=(',', ':'),
                                                 allow_nan=False).encode()).hexdigest()
            require(cell['trained_connection_hash'] == connection_digest(connections), 'trained hash')
            require(pair['natural']['initial_connection_hash'] == connection_digest(gained), 'gained hash')
            require(pair['natural']['probe_connection_hash_before'] == connection_digest(gained), 'natural isolation')
            cut = copy.deepcopy(gained)
            cut_changes = []
            for edge in cut:
                if (edge['source_id'] < 36) != (edge['target_id'] < 36):
                    cut_changes.append({'edge': [edge['source_id'], edge['target_id']], 'old_weight': edge['weight']})
                    edge['weight'] = 0.0
            require(pair['boundary_zero']['boundary_interventions'] == cut_changes, 'cut isolation records')
            require(pair['boundary_zero']['probe_connection_hash_before'] == connection_digest(cut), 'cut isolated hash')
            # World route membership is supplied by the verifier's fixed audit, never scores.
            world = next(w for w in runner.development_worlds(runner.ScaleStudyConfig())
            ...
            require(pair['boundary_visible_trace_equal'] is
                    (pair['natural']['visible_trace'] == pair['boundary_zero']['visible_trace']), 'trace comparison')
            require(pair['boundary_visible_final_state_equal'] is
                    (pair['natural']['visible_final_state'] == pair['boundary_zero']['visible_final_state']), 'state comparison')
            baseline = by_identity[(1.0, index)]
            for key in ('spikes', 'arrival_observations', 'hidden_units', 'visible_final_state',
                        'observed_state_hash', 'reference_state_hash', 'probe_connection_hash_before'):
                require(pair['boundary_zero'][key] == baseline['boundary_zero'][key],
                        'cut controls differ across gain')
            if gain == 1.0:
                prior = old_cell['route_probes'][index]['natural']
                require(all(pair['natural'][key] == value for key, value in prior.items()),
                        'baseline not byte-equivalent to retained RD001 natural')
                baseline_matches += 1
    require(sum(len(v) for v in groups.values()) == 468 and baseline_matches == 78,
            'probe completeness')
    require(sum(r['wall_seconds'] for r in rows) <= 600, 'total wall ceiling')
    conditions = {f'{gain:g}:{name}': {
        'probes': len(values),
        'hidden_spiking_probes': sum(m['hidden_spikes'] > 0 for _, m in values),
        'hidden_spikes': sum(m['hidden_spikes'] for _, m in values),
        'strict_exact_probes': sum(p['behavior']['exact_sequence_recovered'] for p, _ in values),
        'raw_contamination': sum(p['behavior']['raw_contamination'] for p, _ in values),
        'excess_route_events': sum(p['behavior']['excess_route_event_count'] for p, _ in values),
    ...
    return {**verified, 'engineering_accepted': True, 'formal_execution_allowed': False,
            'source_git_sha': manifest['source_git_sha'], 'baseline_exact_matches': baseline_matches,
