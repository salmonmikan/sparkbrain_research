...
def _validate_input(payload: dict[str, Any]) -> None:
    ...
    for key in (
        "left_activity", "right_activity", "plural_activity",
        "left_boundary_return", "right_boundary_return",
        "left_competition_probe", "right_competition_probe",
    ):
...
def _claim_acquisition(source_sha: str) -> str:
    ...
    result = subprocess.run(
        ...
        input=message, check=True, text=True, capture_output=True, env=env,
    )
...
def _candidate_measurement(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    transplanted = DistributedFieldTraceState.from_field_carrier(
        confirmed.export_field_carrier()
    )
    ...
    try:
        confirmed.apply_external_world_return(
            left_r, sign=1, evidence_id="science:left:confirm", evidence_ledger=restored
        )
    ...
    checks = {
        ...
        "lineage_swap_anonymous_selectivity": (
            confirmed.competition_score(left_q) > confirmed.competition_score(right_q)
            and right_confirmed_state.competition_score(right_q)
            > right_confirmed_state.competition_score(left_q)
        ),
        ...
        "dedup_checkpoint": duplicate_rejected
        and restored.export_consumed_ids() == checkpoint,
    }
    signature = {
        "left_pre": left_pre,
...
def _recurrent_null_measurement(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    def score(c: list[float], probe: list[float]) -> float:
...
def _explicit_null_measurement(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "resource_profile": {
            **recurrent["resource_profile"],
            "lookup_privilege_rank": 1,
        },
...
def _belief_null_measurement(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "resource_profile": {
            **recurrent["resource_profile"],
            "persistent_scalars": recurrent["resource_profile"]["persistent_scalars"] + 2,
            "lookup_privilege_rank": 2,
        },
