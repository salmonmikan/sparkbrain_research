#!/usr/bin/env python3
"""Preserve NI01 target-blind raw, manifest, and target-free inventory atomically."""
...
def main() -> None:
    ...
    if not all(path.is_file() for path in files):
        raise SystemExit("NI01 preservation inputs must be regular files")
    if args.destination.is_absolute() or ".." in args.destination.parts:
        raise SystemExit("NI01 destination must be a safe relative path")
    if _remote_branch_sha("origin", args.branch) is not None:
        raise SystemExit("NI01 preservation branch already exists")
    ...
    for path in files:
        ...
        if _sha256(destination) != digests[path.name]:
            raise SystemExit("NI01 copied-byte digest mismatch")
        staged.append(str(destination))
    _git("add", "--", *staged)
    actual = set(_git("diff", "--cached", "--name-only").stdout.splitlines())
    if actual != set(staged):
        raise SystemExit("NI01 unexpected preservation paths")
    _git("commit", "-m", "preserve(ni01): retain target-blind formal raw")
    ...
    if _remote_branch_sha("origin", args.branch) != preservation_commit:
        raise SystemExit("NI01 remote preservation ref mismatch")
