#!/usr/bin/env python3
"""Preserve H5 formal raw, manifest, and input inventory atomically."""
...
def _remote_branch_sha(remote: str, branch: str) -> str | None:
    ...
    if len(lines) != 1:
        raise SystemExit(
            f"ambiguous remote branch resolution for {remote}/{branch}"
        )
...
def main() -> None:
    ...
    if not all(path.is_file() for path in files):
        raise SystemExit("H5 preservation inputs must be regular files")
    if args.destination.is_absolute() or ".." in args.destination.parts:
        raise SystemExit("H5 destination must be a safe relative path")
    if _remote_branch_sha("origin", args.branch) is not None:
        raise SystemExit("H5 preservation branch already exists")
    ...
    for path in files:
        ...
        if _sha256(destination) != digests[path.name]:
            raise SystemExit("H5 copied-byte digest mismatch")
        staged.append(str(destination))
    _git("add", "--", *staged)
    actual = set(_git("diff", "--cached", "--name-only").stdout.splitlines())
    if actual != set(staged):
        raise SystemExit("H5 unexpected preservation paths")
    _git("commit", "-m", "preserve(h5): retain formal raw before scoring")
    ...
    if _remote_branch_sha("origin", args.branch) != preservation_commit:
        raise SystemExit("H5 remote preservation ref mismatch")
