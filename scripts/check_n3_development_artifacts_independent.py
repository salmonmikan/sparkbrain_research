...
def audit(output: Path):
    ...
    for row, (evidence, delay, ancestry, arm) in zip(rows, expected, strict=True):
        ...
        assert row["arm"] == arm
        assert hashlib.sha256(canonical(row["inputs"])).hexdigest() == row["admissible_input_sha256"]
        ...
        for unavailable in ("shared_router_operations", "exact_transient_peak", "resident_duplicates"):
            assert row["resources"][unavailable] is None
        if evidence in ("absence", "replay"):
            assert row["learned_unchanged"] and row["learned_before_sha256"] == row["learned_after_sha256"]
        ...
        if evidence in ("confirmation", "contradiction"):
            assert row["resolution"]["status"] == ("exact-match" if evidence == "confirmation" else "exact-contradiction")
        if arm == "N3":
    ...
    for (a, b), comparison in zip(pairs, summary["comparisons"], strict=True):
        ...
        assert comparison == dict(case_id=a["case_id"], a01=a["confidence_after"],
                                  n3=b["confidence_after"], equal=a["confidence_after"] == b["confidence_after"])
