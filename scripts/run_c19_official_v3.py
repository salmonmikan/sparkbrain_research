...
from sparkbrain.v03_external_validation.official_execution_v3 import (
    ...
    reconstruct_raw_jsonl_v3,
    runtime_manifest_v3,
    write_raw_jsonl_no_clobber_v3,
)
from sparkbrain.v03_external_validation.official_io_v3 import (
...
from sparkbrain.v03_external_validation.official_scoring_v3 import (
...
def acquire_raw(args: argparse.Namespace) -> None:
    ...
    if not args.started_ref.startswith("control/c19-official-v3-started-"):
        raise ValueError("STARTED ref is outside the bound C19-v3 control namespace")
    ...
    harness = OneWayExecutionHarnessV3(boundary=inherited.RuntimeBoundary())
    with network_blocked():
        raw = harness.acquire(
            ...
            raw_writer=lambda bundle: write_raw_jsonl_no_clobber_v3(args.raw, bundle),
        )
...
def score_preserved(args: argparse.Namespace) -> None:
    ...
    raw = reconstruct_raw_jsonl_v3(args.raw, expected_sha256=str(manifest.get("raw_sha256", "")))
    ...
    with network_blocked():
        result = score_official_v3(raw, evaluator_targets)
    output = args.output
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="C19 runtime-closed official-v3 one-way runner")
