...
from sparkbrain.v03_external_validation.official_protocol_v4 import REQUIRED_TORCH_VERSION
...
def _run_synthetic_acquisition_smoke() -> None:
    captured: list[str] = []
    harness = OneWayExecutionHarnessV4(boundary=inherited_execution.RuntimeBoundary())
    raw = harness.acquire(
        admission=ExecutionAdmissionV4.synthetic_dev(),
    ...
    if captured != [raw.sha256]:
        raise RuntimeError(
            "synthetic acquisition did not cross the bound v4 raw-writer boundary exactly once"
        )
...
def main() -> None:
    ...
    with network_blocked():
        ...
        runpy.run_path(
            str(ROOT / "scripts/run_c19_official_v4.py"),
            run_name="c19_official_v4_runner_smoke",
        )
        _run_synthetic_acquisition_smoke()

    print("C19 official-v4 runtime closure and synthetic acquisition smoke: PASS")
