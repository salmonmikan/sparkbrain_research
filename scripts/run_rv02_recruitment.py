...
def verify_bundle(output: Path, source_root: Path = ROOT) -> dict:
    ...
    for row in rows:
        ...
        hidden_trace_count = sum(
            unit not in world["ports"]
            for training in result["training_rows"]
            for unit in training["external_trace_units"]
        )
        if (
            hidden_trace_count != result["hidden_external_trace_count"]
            or hidden_trace_count != 0
            or result["changed_hidden_edges"]
        ):
        ...
        for index, route in enumerate(result["route_probes"]):
            ...
            natural, extended, cut = (route[k] for k in ("natural", "extended", "boundary_zero"))
            for key, horizon, ablated in (
                ("natural", 40.0, False),
                ("extended", 160.0, False),
                ("boundary_zero", 40.0, True),
            ):
                probe = route[key]
                if (
                    probe["horizon_ms"] != horizon
                    or probe["cut_hidden_boundary"] is not ablated
                    or probe["cue_unit"] != world["routes"][index][0]
                ):
                    raise ValueError("probe condition contract mismatch")
            prefix = [spike for spike in extended["spikes"] if spike["time_ms"] <= 140.0]
        ...
        positive_pass = any(
            s["unit_id"] == min_hidden and s["time_ms"] == 100.0 for s in positive["spikes"]
        )
        if (
            result["positive_control_passed"] is not True
            or not positive_pass
            or positive["cue_unit"] != min_hidden
            or positive["horizon_ms"] != 40.0
...
def main() -> int:
    ...
    manifest = {
        "protocol": "rv02-rd001-development",
