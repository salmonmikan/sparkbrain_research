...
from sparkbrain.v03_external_validation.official_execution_v2 import (
    OFFICIAL_SCOPE,
    ExecutionAdmissionV2,
    OneWayExecutionHarnessV2,
    reconstruct_raw_jsonl_v2,
    runtime_manifest_v2,
    write_raw_jsonl_no_clobber_v2,
)
...
def acquire_raw(args: argparse.Namespace) -> None:
    ...
    if not args.started_ref.startswith("control/c19-official-v2-started-"):
        raise ValueError("STARTED ref is outside the bound C19-v2 control namespace")
    ...
    harness = OneWayExecutionHarnessV2(boundary=inherited.RuntimeBoundary())
    with network_blocked():
        raw = harness.acquire(
            ...
            raw_writer=lambda bundle: write_raw_jsonl_no_clobber_v2(args.raw, bundle),
        )
...
def score_preserved(args: argparse.Namespace) -> None:
    ...
    raw = reconstruct_raw_jsonl_v2(args.raw, expected_sha256=expected_sha256)
    ...
    with network_blocked():
        result = score_official_v2(raw, evaluator_targets)

    output = args.output
    output.mkdir(parents=True, exist_ok=False)
    _write_json_no_clobber(output / "scored_predictions.json", result["scored_predictions"])
    _write_json_no_clobber(output / "metrics_by_row.json", result["metrics_by_row"])
    _write_json_no_clobber(output / "paired_statistics.json", result["paired_statistics"])
    _write_json_no_clobber(output / "baseline_matching.json", result["baseline_matching"])
    _write_json_no_clobber(output / "failure_examples.json", result["failure_examples"])
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="C19 fresh-v2 official one-way runner")
