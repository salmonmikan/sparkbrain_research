"""Target-free source-to-cluster binding for prospective C19-R1 inference."""
...
from sparkbrain.v03_external_validation.c19_r1_protocol import EXPECTED_PAIRS
...
def _canonical_payload(entries: Sequence[Mapping[str, Any]]) -> dict[str, object]:
...
def build_atomic_idx_source_map(pairs: Sequence[BeliefRPair]) -> tuple[SourceMapEntry, ...]:
    if len(pairs) != EXPECTED_PAIRS:
        raise ValueError("R1 atomic_idx source map requires exactly 1744 pairs")
    entries = []
    for pair_index, pair in enumerate(pairs):
        before = pair.time_t.atomic_idx
        after = pair.time_t1.atomic_idx
        if not before.strip() or before != after:
            raise ValueError("R1 pair must bind to one non-empty atomic_idx cluster")
...
def atomic_idx_clusters(
    ...
    if not clusters:
        raise ValueError("R1 atomic_idx source map has no clusters")
...
def read_atomic_idx_source_map(
    ...
    *,
    ...
    if actual_sha256 != expected_sha256:
        raise ValueError("R1 atomic_idx source-map digest mismatch")
    value = json.loads(payload.decode("utf-8"))
    if not isinstance(value, Mapping):
        raise ValueError("R1 atomic_idx source-map artifact must be a mapping")
    if value.get("schema_version") != SOURCE_MAP_SCHEMA_VERSION:
        raise ValueError("R1 atomic_idx source-map schema drift")
    if value.get("cluster_key") != SOURCE_MAP_CLUSTER_KEY:
        raise ValueError("R1 atomic_idx source-map cluster key drift")
    if value.get("pair_assignment") != "exactly_once":
        raise ValueError("R1 atomic_idx source-map assignment contract drift")
    if value.get("target_fields_materialized") is not False:
        raise ValueError("R1 atomic_idx source map must remain target-free")
    entries = value.get("entries")
    if not isinstance(entries, list) or not all(isinstance(entry, Mapping) for entry in entries):
        raise ValueError("R1 atomic_idx source-map entries missing")
    normalized = validate_atomic_idx_source_map(entries)
    if _artifact_bytes(normalized) != payload:
        raise ValueError("R1 atomic_idx source-map artifact is not canonical")
    return normalized
