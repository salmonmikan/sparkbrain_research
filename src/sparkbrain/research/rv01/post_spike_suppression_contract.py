...
R01_15_PROTOCOL_ID = "rv01-r01-15-post-spike-suppression-v1"
R01_15_DEVELOPMENT_SEEDS = (141500, 141501, 141502, 141503, 141504)
R01_15_HELD_OUT_SEEDS = tuple(range(141600, 141610))
_R01_15_WORLD_SALT = "rv01-r01-15-fresh-world-grid-v1"
...
class PostSpikeSuppressionWorldSpec:
    ...
    def validate(self) -> None:
        allowed = (
            ...
            else R01_15_HELD_OUT_SEEDS
        )
        if self.seed not in allowed:
            raise ValueError("seed does not belong to the R01-15 phase")
        if self.unit_count != 96:
            raise ValueError("R01-15 fixes the world scale at 96 anonymous units")
        if len(self.routes) < 2:
            raise ValueError("R01-15 requires multiple routes")
        for route in self.routes:
            route.validate(unit_count=self.unit_count)
        route_ids = tuple(route.route_id for route in self.routes)
        if len(set(route_ids)) != len(route_ids):
            raise ValueError("route IDs must be unique")
        if len(self.training_order) != len(route_ids) or set(self.training_order) != set(route_ids):
    ...
    def world_id(self) -> str:
        return f"r01-15:{self.phase.value}:{self.family.value}:{self.seed}"
...
def post_spike_suppression_world(
    phase: InterferencePhase,
    family: InterferenceFamily,
    seed: int,
    ...
    if seed not in allowed:
        raise ValueError("unsupported R01-15 seed for phase")
...
def development_post_spike_suppression_worlds() -> tuple[PostSpikeSuppressionWorldSpec, ...]:
    return tuple(
        post_spike_suppression_world(InterferencePhase.DEVELOPMENT, family, seed)
        for family in InterferenceFamily
        for seed in R01_15_DEVELOPMENT_SEEDS
    )


def held_out_post_spike_suppression_worlds() -> tuple[PostSpikeSuppressionWorldSpec, ...]:
    return tuple(
        post_spike_suppression_world(InterferencePhase.HELD_OUT, family, seed)
        for family in InterferenceFamily
        for seed in R01_15_HELD_OUT_SEEDS
    )


def post_spike_suppression_world_grid_hash(
    worlds: tuple[PostSpikeSuppressionWorldSpec, ...],
) -> str:
...
__all__ = [
    ...
    "held_out_post_spike_suppression_worlds",
