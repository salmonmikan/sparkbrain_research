...
R01_13_PROTOCOL_ID = "rv01-r01-13-activity-matched-discrimination-v1"
R01_13_DEVELOPMENT_SEEDS = (200, 201, 202, 203, 204)
R01_13_HELD_OUT_SEEDS = tuple(range(300, 310))
_R01_13_WORLD_SALT = "rv01-r01-13-fresh-world-grid-v1"
...
class ActivityMatchedWorldSpec:
    ...
    def validate(self) -> None:
        allowed = (
            ...
            else R01_13_HELD_OUT_SEEDS
        )
        if self.seed not in allowed:
            raise ValueError("seed does not belong to the R01-13 phase")
        if self.unit_count != 96:
            raise ValueError("R01-13 fixes the world scale at 96 anonymous units")
        if len(self.routes) < 2:
            raise ValueError("R01-13 requires multiple routes")
    ...
    def world_id(self) -> str:
        return f"r01-13:{self.phase.value}:{self.family.value}:{self.seed}"
...
def activity_matched_world(
    ...
    if seed not in allowed:
        raise ValueError("unsupported R01-13 seed for phase")
...
def development_activity_matched_worlds() -> tuple[ActivityMatchedWorldSpec, ...]:
    return tuple(
        activity_matched_world(InterferencePhase.DEVELOPMENT, family, seed)
        for family in InterferenceFamily
        for seed in R01_13_DEVELOPMENT_SEEDS
    )


def held_out_activity_matched_worlds() -> tuple[ActivityMatchedWorldSpec, ...]:
    return tuple(
        activity_matched_world(InterferencePhase.HELD_OUT, family, seed)
        for family in InterferenceFamily
        for seed in R01_13_HELD_OUT_SEEDS
    )


def activity_matched_world_grid_hash(
    worlds: tuple[ActivityMatchedWorldSpec, ...],
) -> str:
