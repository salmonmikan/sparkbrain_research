...
from sparkbrain.research.rv02_scale import ScaleStudyConfig, audit_scale, project_behavior, digest
...
def apply_boundary_gain(field: TemporalExcitableField, gain: float
...
def score_probe(spikes: list[dict[str, Any]], route: tuple[int, ...],
                unit_count: int) -> dict[str, Any]:
    """Score continuation only; forced cue and hidden spikes are not symbols."""
    generated = tuple(s["unit_id"] for s in spikes if s["time_ms"] > 100.)
    ...
    result["missing_expected_count"] = len(route[1:])-round(result["ordered_retention"]*len(route[1:]))
...
def run_probe(field: TemporalExcitableField, cue_unit: int, *, horizon_ms: float = 40.,
    ...
    for runner in (observed, reference):
        try:
            runner.run_until(100.+horizon_ms)
            errors.append(None)
        except RuntimeError as exc:
            if str(exc) not in ("max_events_per_run exceeded", "max_spikes_per_run exceeded"):
                raise
            errors.append(str(exc))
    if (errors[0] != errors[1] or observed.state_hash() != reference.state_hash()
    ...
    if errors[0] is not None or observed.total_arrivals >= 4096 or observed.total_spikes >= 512:
        return {"status": "incomplete_native_guard", "error": errors[0] or "native limit reached",
                "metrics_available": False, "actual_arrivals": observed.total_arrivals,
                "actual_spikes": observed.total_spikes, "final_clock_ms": observed.current_time_ms,
                "final_queue_size": len(observed.state_dict()["queue"]),
                "partial_state": observed.state_dict(), "spikes": observed.delivered_spikes,
                "arrival_observations": observed.arrival_observations,
                "reference_error": errors[1], "reference_spikes": [asdict(s) for s in reference.delivered_objects],
                "probe_connection_hash_before": probe_connection_hash,
                "probe_connection_hash_after": connection_hash(observed),
                "observed_state_hash": observed.state_hash(), "reference_state_hash": reference.state_hash(), "observer_equivalence": observed.state_hash()==reference.state_hash()}
...
def run_boundary_cell(config: ScaleStudyConfig, world: dict[str, Any],
    ...
    for route_index, (route, exposures) in enumerate(
        ...
        for episode in range(exposures):
            ...
            for index, unit in enumerate(route):
                ...
                training_rows.append({"route_index": route_index, "episode": episode,
                    "position": index, "external_unit": unit,
                    "external_trace_units": sorted(map(int, learner.state_dict()["unit_traces"])),
                    "updates": [asdict(update) for update in updates]})
    changed_hidden = [list(key) for key, edge in field.connections.items()
    ...
    for gain in GAINS:
        ...
        for index, route in enumerate(world["routes"]):
            pair = {"gain": gain, "route_index": index,
                    "gain_interventions": interventions}
            for name, cut in (("natural", False), ("boundary_zero", True)):
                try:
                    ...
                    if str(exc) not in ("max_events_per_run exceeded", "max_spikes_per_run exceeded"):
                        raise
                    pair[name] = {"status": "incomplete_native_guard", "error": str(exc),
            ...
            pair["boundary_visible_final_state_equal"] = (
                pair["natural"]["visible_final_state_hash"] == pair["boundary_zero"]["visible_final_state_hash"]
                if complete else None)
            probes.append(pair)
    return {"diagnosis": "rv02-rd002-development", "audit": audit,
            "training_rows": training_rows, "changed_hidden_edges": changed_hidden,
            "hidden_external_trace_count": sum(u not in PORTS for row in training_rows
                                               for u in row["external_trace_units"]),
            "trained_state": field.state_dict(),
            "trained_connection_hash": connection_hash(field), "route_probes": probes,
