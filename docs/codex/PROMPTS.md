...
acceptance-history documents.

Every new prompt inherits `docs/LOCAL_EXECUTION_POLICY.md`: CPU reference path, local storage/UI,
no mandatory remote runtime service, and no dedicated-hardware acceptance criterion. Read
...
local readiness. Do not broaden the task into a new scientific evaluation.
```

## Historical task records

- `C01_ENGINE_HARDENING.md` through `C10_REPRODUCIBILITY_RELEASE.md` document the completed
  v0.2.1 sequence.
- C11--C20 outcomes, including negative and blocked results, are recorded in
  `docs/PROJECT_STATUS.md`, `docs/RESULTS_LEDGER.md`, and `docs/EXPERIMENT_PROTOCOL.md`.
- A future C19 official evaluation requires a new preregistered protocol; the blocked-readiness
  bundle cannot be converted into an execution prompt.
