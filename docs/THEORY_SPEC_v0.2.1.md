# Spark Dynamic Cognition — Theory Specification v0.2.1
...
> **入門者向け:** 数式や英語用語に慣れていない場合は、先に `FOUNDATIONS_FOR_BEGINNERS.md` と `GLOSSARY.md` を読む。
...
コア成果物は、一台の汎用ローカルコンピューター上で、外部APIやクラウドサービスを必須とせず実行・観察・保存・比較できることを正式条件とする。CPU参照経路を必須とし、ローカルGPUとローカルSNN simulationは任意の高速化・拡張として認める。専用ニューロモーフィックハードウェアは本仕様の完成条件から分離し、独立したExtension Hとして扱う。詳細は `LOCAL_EXECUTION_POLICY.md` を参照する。
...
v0.2.1 reference score（数式はv0.2から変更なし）:
...
v0.2.1 では最小実装のみ。これはend-to-end学習の証明ではない。
...
8. core reference behaviorはCPUのみのローカル環境で再現可能である。
9. 通常実行は外部API、クラウドDB、遠隔ストレージを必須としない。
10. UIは静的ファイルまたはlocalhostで動作し、正式配布物は外部CDNを必須としない。
11. algorithmic sparsityと実消費電力を区別する。energy claimは独立した専用hardware研究まで保留する。
12. package patch versionとpersisted schema versionを区別する。v0.2.1のschemaは0.2を維持する。
...
## 16. Local execution contract

本理論は、次の三層を区別する。

1. **Reference semantics:** Pythonの決定論的CPU実装で再現する、理論上の標準挙動。
2. **Local accelerated semantics:** 同一PC上のPyTorch、GPU、SNN simulator等で、事前定義した許容差内の挙動を目指す実装。
3. **Dedicated hardware extension:** Loihi、FPGA、ASIC等への写像と実電力測定。コア理論完成とは独立に評価する。

ローカル高速版やspiking版が参照版と異なる場合、参照版を暗黙に変更せず、差分をtrace、test、decision logへ記録する。

## 17. Versioning

- theory/package release: `0.2.1`
- persisted config/state/trace schema: `0.2`
- dynamics equations: v0.2から意図的変更なし
- 主な変更: ローカル完結条件、平易な導入文書、用語定義、専用hardware分離
