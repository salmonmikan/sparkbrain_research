...
## A Working Technical Report for SparkBrain v0.2.1
...
The core completion target is one general-purpose local computer. The deterministic reference engine, worlds, trace/checkpoint storage, visualizer, experiments, and reporting must run without mandatory cloud services or remote model APIs. A CPU reference path is required; local GPU acceleration and local spiking simulators are optional.

Dedicated neuromorphic hardware, FPGA/ASIC mapping, and physical energy measurement are separated into independent Extension H. They may test an additional hardware-efficiency hypothesis, but they are not prerequisites for completing the core theory or software system.

This is a methodological constraint as well as an operational one: causal state, experimental inputs, interventions, and raw outputs remain under the experimenter's local control, and a proprietary service cannot silently change the semantics of the reference result.
...
The complete working equations and semantics are defined in `THEORY_SPEC_v0.2.1.md`. A plain-language introduction is provided in `FOUNDATIONS_FOR_BEGINNERS.md`, with term-by-term clarification in `GLOSSARY.md`.

## 5. Reference implementation

The v0.2.1 Python implementation uses a deterministic priority queue. Each event is ordered by time, priority, and insertion sequence. Dormant Sparks are not updated every global tick. The reference engine records:
