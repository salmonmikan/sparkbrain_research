...
Independent review of the first verifier revision found that retained `topology_sha256` and connection rows were self-authenticating rather than independently regenerated. That blocker has now been repaired and merged before this reconciled package branch was created. The active base regenerates canonical topology and pre-training gained connection state from the fixed seed/world/scale construction, and rejects forged retained topology/connection digests.

The package contract includes the repaired verifier in `RD005_REQUIRED_SOURCE_PATHS`, so any eventual exact source manifest must bind that reviewed verifier revision or a later separately reviewed successor.

## Remaining safe prerequisites

Before any RD005 development capability execution can be considered, the line still needs, in order:

1. construct and audit the authoritative consumed/reserved seed + world registry;
2. build a real SHA-256 source manifest at an exact candidate commit;
3. bind an exact Python runtime and **construction-only** command contract;
4. create a fresh-output/no-clobber construction-only wrapper that always preserves raw D1 output even on failure and contains no learner/probe code path;
5. CI/review that exact construction source and freeze it under a non-moving development construction source ref;
6. run D1 construction only, retain the canonical artifact, and independently verify/review that exact artifact identity;
7. only after D1 review, prepare a separate capability package/runner with outcome-blind scoring fixed before capability output opens.
